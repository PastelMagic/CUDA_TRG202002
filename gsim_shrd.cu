...
__global__ void GPUmove()
{
	...
	__shared__ double Sh_posX[OBJS], Sh_posY[OBJS];	// シェアードメモリ
	...
	if (thread < NThreads) {		// スレッド数未満なら担当分あり
		Start = thread * Size;
		End = Start + Size;
		for (int i=Start; i<End; i++) {	// シェアードメモリ領域に担当分をコピー
			Sh_posX[i] = posX[i];
			Sh_posY[i] = posY[i];
		}
		__syncthreads();		// 全員終わるまで待つ
		...
		for (int i=Start; i<End; i++) {
			...
			for (int j=1; j<Nobj; j++) {		// 自分との関係は計算しなくていい
				...
				double dfX = Sh_posX[targ] - Sh_posX[i];	// X方向の距離
				double dfY = Sh_posY[targ] - Sh_posY[i];	// Y方向の距離
